document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // PROMPT LIBRARY SEARCH WITH AUTOCOMPLETE
    // ==========================================
    ...
    if (searchInput) {
        ...
        // Build prompt index by category
        const promptIndex = [];
        const categories = {
        ...
        exampleBlocks.forEach(block => {
            const titleEl = block.querySelector('.example-title');
            const tabContent = block.closest('.tab-content');
            if (titleEl && tabContent) {
        ...
        function showSuggestions(query) {
            ...
            if (query.length === 0) {
                // Show all prompts by category
                const grouped = {};
                promptIndex.forEach(item => {
                ...
                Object.keys(grouped).forEach(cat => {
                    ...
                    categoryDiv.textContent = categories[cat] || cat;
                    autocomplete.appendChild(categoryDiv);

                    grouped[cat].forEach(item => {
                        const suggestion = createSuggestion(item);
                        autocomplete.appendChild(suggestion);
                    });
                });
            } else {
                // Filter by query
                const matches = promptIndex.filter(item =>
                    item.title.toLowerCase().includes(query) ||
                    item.categoryName.toLowerCase().includes(query)
                );

                if (matches.length === 0) {
                    autocomplete.innerHTML = '<div class="search-suggestion">No matches found</div>';
                } else {
        ...
        function createSuggestion(item) {
            ...
            div.innerHTML = `
                ...
                <div class="search-suggestion-context">${item.categoryName}</div>
            `;
            div.addEventListener('click', () => {
                searchInput.value = item.title;
                autocomplete.classList.remove('active');
                filterPrompts(item.title.toLowerCase());
        ...
        function filterPrompts(query) {
            ...
            if (query !== '' && visibleCount === 0) {
                ...
                noResultsMsg.textContent = `No prompts found matching "${query}"`;
